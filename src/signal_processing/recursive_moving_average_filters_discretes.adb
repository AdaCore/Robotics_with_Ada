package body Recursive_Moving_Average_Filters_Discretes is
   ...
   begin
      if Empty (This.Samples) then
         ...
         This.Total := Accumulator (Sample'Pos (New_Sample));
      ...
      if Full (This.Samples) then -- delete the oldest sample
         ...
         This.Total := This.Total - Accumulator (Sample'Pos (Next_Element_Out (This.Samples)));
         Delete (This.Samples, Count => 1);
      end if;

      Put (This.Samples, New_Sample);

      This.Total := This.Total + Accumulator (Sample'Pos (New_Sample));
